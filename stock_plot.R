# Downloaded from: http://addictedtor.free.fr/graphiques/graphcode.php?graph=65
...
plotBollingerBars <- function(X) {
  ...
  oldpar <- par(mar=c(0,4,2,4),         # no bottom spacing 
                ...
                lend="butt")            # butt line ends

  ## set up coordinates
  plot.window(c(0.5,NROW(X)+0.5), range(X[,c("Open","Low","High","Close", 
  ...
  ##      for losing days where close is lower than open, plot
  ##	    red bars showing retreat from the open to the close
  ind <- which(X$Close > X$Open) 
  col <- rep("red", length(x))
  col[ind] <- "green"
  segments(x, X[,"Open"], x, X[,"Close"], col=col, lwd=5)

  ## part three: blue bars for the days intraday retreat from the lower
...
plotBollingerIndicators <- function(X) {
  ...
  plot(x, bbwidth, col='blue', ylab="", type='l', axes=FALSE, xaxs="i", 
       xlim=c(0.5,NROW(X)+0.5),ylim=c(min(0, min(bbwidth)) ,max(bbwidth))) 
...
plotVolumeBars <- function(X) {
  ...
  plot.new()                            # empty plot 
  plot.window(c(0.5,NROW(X)+0.5), range(X$scaledvol), xaxs="i")     # set up coordinates 
  grid()                                # dashed grid 
